...
class ResUsers(models.Model):
    ...
    def SELF_READABLE_FIELDS(self):
        return super().SELF_READABLE_FIELDS + ["store_ids"]

    @property
    def SELF_WRITEABLE_FIELDS(self):
        return super().SELF_WRITEABLE_FIELDS + ["store_ids"]
        ...
        if group_multi_store and "store_ids" in values:
            for user in self:
                if len(user.store_ids) <= 1 and user.id in group_multi_store.users.ids:
                    ...
                    len(user.store_ids) > 1
                    and user.id not in group_multi_store.users.ids
                ):
                    user.write({"groups_id": [(4, group_multi_store.id)]})
        return res
